...
impl<'buf,const HL:usize> HttpHeaders<'buf,HL>{
    ...
    pub(crate) fn new(bytes: &'buf [u8]) -> Result<HttpHeaders<'buf, HL>, CreatingHeadersErrors> {
        ...
        use std::arch::x86_64::*;
        let mut lines: [HeaderLine<'buf>; HL] = unsafe { std::mem::MaybeUninit::uninit().assume_init() };
        let mut lines_index = 0_usize;
        ...
        unsafe {
            while ptr < end_ptr {
                ...
                // 2. SIMD Colon Search (16 bytes at once)
                let mut colon_idx = None;
                if remaining >= 16 {
                    let chunk = _mm_loadu_si128(ptr as *const __m128i);
                    let mask = _mm_movemask_epi8(_mm_cmpeq_epi8(chunk, _mm_set1_epi8(b':' as i8))) as u32;
                    if mask != 0 { colon_idx = Some(mask.trailing_zeros() as usize); }
                }

                let c_pos = match colon_idx {
                    Some(i) => i,
                ...
                let key = std::slice::from_raw_parts(ptr, c_pos);
                let mut val_ptr = ptr.add(c_pos + 1);
                if val_ptr < end_ptr && *val_ptr == b' ' { val_ptr = val_ptr.add(1); }

                // 3. SIMD Newline Search (16 bytes at once)
                ...
                if val_rem >= 16 {
                    ...
                    if mask != 0 { newline_idx = Some(mask.trailing_zeros() as usize); }
                }

                let lf_idx = match newline_idx {
                    Some(i) => i,
                ...
                let value_bytes = std::slice::from_raw_parts(val_ptr, v_len);

                // 4. CRITICAL: Use ptr::write to avoid dropping garbage memory
                if lines_index < HL {
